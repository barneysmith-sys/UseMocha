...
export default async function handler(req, res) {
  ...
  if (cleanMode === 'followup') {
    ...
    prompt = [
      ...
      ``,
      `Rules: ALWAYS use exactly ONE decimal place (6.4 not 6, 7.0 not 7). Be a rigorous ${rubric.firms} interviewer — most first attempts score 4.5–6.5. 8.0+ only if genuinely impressive. Calibration: ${rubric.scoreGuide}.`,
      `The improved_answer MUST be 5-7 complete sentences in first person — specific, quantified, with I-language throughout. NEVER bullet points. Write at ${rubric.shortName} offer standard. Even if the answer is short or weak, write a FULL model answer showing exactly what a top candidate would say. This is the most important field — make it exceptional.`,
      ``,
      `Respond ONLY with valid JSON, no markdown fences:`,
      `{"star_breakdown":{"situation":"<present/MISSING — if present: what was strong + what specific detail to add; if MISSING: exact sentence to add>","task":"<clear/unclear+fix>","action":"<I-language present/absent — exact actions taken + what specific I-statement to add or strengthen>","result":"<quantified/vague/MISSING — specific number or outcome present? + exact improvement or example result to add>","weak_components":"<comma-separated list of the 1-3 weakest STAR components>","strengths":"<comma-separated list of 2-4 genuine strengths — specific, e.g. 'Quantified result', 'Strong I-language', 'Clear situation', 'Commercial insight'>"},"industry_critique":"<3-4 sentences grading against the specific framework: 1) what was present and strong, 2) the exact dimension that cost the most points + why, 3) the specific technique top candidates use that was absent, 4) calibrated comparison to what a 9.0+ answer includes — direct, no sugarcoating>","improved_answer":"<5-7 complete sentences at 9.0+ standard — starts with situation (when/where/who), specific task I owned, 2-3 concrete actions using I-language, quantified result, reflection on what I learned — ${rubric.shortName} voice — no bullet points — reads like a confident candidate answer>","interviewer_perspective":"<1 sentence — yes/borderline/no at ${rubric.firms} + main reason>","interviewer_verdict":"<'Would advance to next round' OR 'On the fence' OR 'Would not advance'>","scores":{"structure":x.x,"clarity":x.x,"ownership":x.x,"impact":x.x},"overall":x.x,"verdict":"<2-4 sentences of direct coaching: (1) name the standard met or missed, (2) the single most important structural fix with exactly how to do it, (3) what the best answers at this firm always include that this one didn't>","share_line":"<one punchy sentence the user can share — score + gap + mocha — e.g. 'I scored 7.8/10 on a McKinsey PEI question. Ownership was my gap. Fixing it with Mocha.'>","next_question":"<one follow-up the interviewer asks based on answer gaps>","framework_used":"${rubric.framework}","firms_standard":"${rubric.firms}","citation":"${rubric.citation}"}`,
    ].join('\n');
  }
