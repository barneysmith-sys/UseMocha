...
export default async function handler(req, res) {
  ...
  if (cleanMode === 'followup') {
    ...
    prompt = [
      ...
      `{"star_breakdown":{"situation":"<present/MISSING — if present: rate strength (strong/weak), name what context was given, then give the exact sentence they should add to strengthen it; if MISSING: write the exact opening sentence they should use>","task":"<clear/unclear/MISSING — state whether their specific responsibility was explicit, then write the exact sentence they should add to clarify their personal ownership>","action":"<I-language present/absent — exact actions taken + what specific I-statement to add or strengthen>","result":"<quantified/vague/MISSING — specific number or outcome present? + exact improvement or example result to add>","weak_components":"<comma-separated list of the 1-3 weakest STAR components>","strengths":"<comma-separated list of 2-4 genuine strengths — specific, e.g. 'Quantified result', 'Strong I-language', 'Clear situation', 'Commercial insight'>"},"industry_critique":"<4-5 sentences of expert grading — name the specific ${rubric.framework} dimensions by name: 1) acknowledge what was genuinely strong and why it works at ${rubric.firms}, 2) identify the single dimension that cost the most points with the exact reason, 3) describe the specific technique top ${rubric.firms} candidates always use that was absent here, 4) compare directly to a 9.0+ answer — what would it have included?, 5) one precise coaching instruction the candidate can follow immediately — start with a verb>","improved_answer":"<MINIMUM 150 WORDS — write the complete answer as the candidate speaking aloud in an interview. Real first-person prose, no brackets, no instructions, no bullet points. Use their details; freely invent realistic specifics where missing. Must include: (1) Situation — specific when/where/company/team, (2) Task — your exact personal responsibility using I-language, (3) Actions — at least three concrete things YOU did: I decided, I built, I led, I proposed, (4) Result — specific quantified outcome with a number or percentage, (5) Reflection — one sentence on what this taught you. This answer must be long enough that a candidate could read it aloud and it would take 60-90 seconds. ${rubric.shortName} voice. NEVER use square brackets or placeholders.>","interviewer_perspective":"<1 sentence — yes/borderline/no at ${rubric.firms} + main reason>","interviewer_verdict":"<'Would advance to next round' OR 'On the fence' OR 'Would not advance'>","scores":{"structure":x.x,"clarity":x.x,"ownership":x.x,"impact":x.x},"overall":x.x,"verdict":"<3-5 sentences of direct, expert coaching — no hedging: (1) state clearly whether this meets ${rubric.firms} standard and why, (2) name the single structural change that would lift the score most, with a specific example of how to phrase it, (3) what every strong ${rubric.firms} answer includes that this one was missing, (4) one concrete thing to add in the next attempt — specific enough to act on immediately>","share_line":"<one punchy sentence the user can share — score + gap + mocha — e.g. 'I scored 7.8/10 on a McKinsey PEI question. Ownership was my gap. Fixing it with Mocha.'>","next_question":"<one follow-up the interviewer asks based on answer gaps>","framework_used":"${rubric.framework}","firms_standard":"${rubric.firms}","citation":"${rubric.citation}"}`,
    ].join('\n');
  }
